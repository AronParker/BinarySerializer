...
{
    internal sealed class Int64ArrayFormatter : IFormatter<long[]>
    {
        public int GetSize(long[] value, int maxArrayLength, int maxRecursionDepth)
        {
            ...
            for (var i = 0; i < value.Length; i++)
                size += Binary.InternalGet7BitEncodedInt64Size(value[i]);
        ...
        public int Serialize(long[] value, byte[] buffer, int offset, int count, int maxArrayLength, int maxRecursionDepth)
        {
            ...
            for (var i = 0; i < value.Length; i++)
            {
                size = Binary.InternalWrite7BitEncodedInt64(value[i], buffer, offset, count);
        ...
        public long[] Deserialize(byte[] buffer, int offset, int count, out int bytesRead, int maxArrayLength, int maxRecursionDepth)
        {
            ...
            var value = length == 0 ? Array.Empty<long>() : new long[length];
            ...
            for (var i = 0; i < value.Length; i++)
            {
                value[i] = Binary.InternalRead7BitEncodedInt64(buffer, offset, count, out size);

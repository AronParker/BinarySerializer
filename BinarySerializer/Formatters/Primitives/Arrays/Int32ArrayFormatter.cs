...
{
    internal sealed class Int32ArrayFormatter : IFormatter<int[]>
    {
        public int GetSize(int[] value, int maxArrayLength, int maxRecursionDepth)
        {
            ...
            for (var i = 0; i < value.Length; i++)
                size += Binary.InternalGet7BitEncodedInt32Size(value[i]);
        ...
        public int Serialize(int[] value, byte[] buffer, int offset, int count, int maxArrayLength, int maxRecursionDepth)
        {
            ...
            for (var i = 0; i < value.Length; i++)
            {
                size = Binary.InternalWrite7BitEncodedInt32(value[i], buffer, offset, count);
        ...
        public int[] Deserialize(byte[] buffer, int offset, int count, out int bytesRead, int maxArrayLength, int maxRecursionDepth)
        {
            ...
            var value = length == 0 ? Array.Empty<int>() : new int[length];
            ...
            for (var i = 0; i < value.Length; i++)
            {
                value[i] = Binary.InternalRead7BitEncodedInt32(buffer, offset, count, out size);

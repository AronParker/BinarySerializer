using System.Reflection.Emit;
using System.Runtime.CompilerServices;

namespace BinarySerializer.Extensions
{
    internal static class ILGeneratorEx
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void EmitLoadArgument(this ILGenerator il, int index)
        {
            switch (index)
            {
                case 0:
                    il.Emit(OpCodes.Ldarg_0);
                    return;
                case 1:
                    il.Emit(OpCodes.Ldarg_1);
                    return;
                case 2:
                    il.Emit(OpCodes.Ldarg_2);
                    return;
                case 3:
                    il.Emit(OpCodes.Ldarg_3);
                    return;
            }

            if (index <= byte.MaxValue)
                il.Emit(OpCodes.Ldarg_S, (byte)index);
            else
                il.Emit(OpCodes.Ldarg, (ushort)index);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void EmitLoadArgumentAddress(this ILGenerator il, int index)
        {
            if (index <= byte.MaxValue)
                il.Emit(OpCodes.Ldarga_S, (byte)index);
            else
                il.Emit(OpCodes.Ldarga, (ushort)index);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void EmitStoreArgument(this ILGenerator il, int index)
        {
            if (index <= byte.MaxValue)
                il.Emit(OpCodes.Starg_S, (byte)index);
            else
                il.Emit(OpCodes.Starg, (ushort)index);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void EmitLoadLocal(this ILGenerator il, int index)
        {
            switch (index)
            {
                case 0:
                    il.Emit(OpCodes.Ldloc_0);
                    return;
                case 1:
                    il.Emit(OpCodes.Ldloc_1);
                    return;
                case 2:
                    il.Emit(OpCodes.Ldloc_2);
                    return;
                case 3:
                    il.Emit(OpCodes.Ldloc_3);
                    return;
            }

            if (index <= byte.MaxValue)
                il.Emit(OpCodes.Ldloc_S, (byte)index);
            else
                il.Emit(OpCodes.Ldloc, (ushort)index);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void EmitLoadLocalAddress(this ILGenerator il, int index)
        {
            if (index <= byte.MaxValue)
                il.Emit(OpCodes.Ldloca_S, (byte)index);
            else
                il.Emit(OpCodes.Ldloca, (ushort)index);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void EmitStoreLocal(this ILGenerator il, int index)
        {
            switch (index)
            {
                case 0:
                    il.Emit(OpCodes.Stloc_0);
                    return;
                case 1:
                    il.Emit(OpCodes.Stloc_1);
                    return;
                case 2:
                    il.Emit(OpCodes.Stloc_2);
                    return;
                case 3:
                    il.Emit(OpCodes.Stloc_3);
                    return;
            }

            if (index <= byte.MaxValue)
                il.Emit(OpCodes.Stloc_S, (byte)index);
            else
                il.Emit(OpCodes.Stloc, (ushort)index);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void EmitLoadDeclaredConstant(this ILGenerator il, int value)
        {
            switch (value)
            {
                case -1:
                    il.Emit(OpCodes.Ldc_I4_M1);
                    return;
                case 0:
                    il.Emit(OpCodes.Ldc_I4_0);
                    return;
                case 1:
                    il.Emit(OpCodes.Ldc_I4_1);
                    return;
                case 2:
                    il.Emit(OpCodes.Ldc_I4_2);
                    return;
                case 3:
                    il.Emit(OpCodes.Ldc_I4_3);
                    return;
                case 4:
                    il.Emit(OpCodes.Ldc_I4_4);
                    return;
                case 5:
                    il.Emit(OpCodes.Ldc_I4_5);
                    return;
                case 6:
                    il.Emit(OpCodes.Ldc_I4_6);
                    return;
                case 7:
                    il.Emit(OpCodes.Ldc_I4_7);
                    return;
                case 8:
                    il.Emit(OpCodes.Ldc_I4_8);
                    return;
            }

            if (value >= sbyte.MinValue && value <= sbyte.MaxValue)
                il.Emit(OpCodes.Ldc_I4_S, (sbyte)value);
            else
                il.Emit(OpCodes.Ldc_I4, value);
        }
    }
}
